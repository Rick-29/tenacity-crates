...
use crate::security::{
    middleware::traits::{ConfigurableEncryptor, VersionTrait},
    TenacityEncryptor, TenacityMiddleware, TenacityMiddlewareStream,
};
...
impl V2Encryptor {
        ...
        P: AsRef<[u8]>,
    {
        ...
        Argon2::default()
            .hash_password_into(secret.as_ref(), &self.salt, &mut output_key)
            .map_err(EncryptorError::Argon2)?;
        Ok(output_key.into())
    }
...
impl TenacityMiddleware for V2Encryptor {
    ...
    {
        anyhow::bail!(
            "Error, Unallowed: Can not encrypt string to string, use `encrypt_bytes` instead"
        )
    }
    ...
    {
        anyhow::bail!(
            "Error, Unallowed: Can not decrypt string to string, use `decrypt_bytes` instead"
        )
    }
...
impl VersionTrait for V2Encryptor {
    ...
    fn encrypt_bytes<P: AsRef<[u8]> + Send, T: ?Sized + AsRef<[u8]>>(
        ...
        let ciphertext_with_tag = cipher
            .encrypt(&self.nonce.into(), bytes.as_ref())
            .map_err(EncryptorError::AesGcmEncryption)?;
    ...
    fn decrypt_bytes<P: AsRef<[u8]> + Send, T: ?Sized + AsRef<[u8]>>(
        ...
        let ciphertext_with_tag = cipher
            .decrypt(&self.nonce.into(), bytes.as_ref())
            .map_err(EncryptorError::AesGcmDecryption)?;
        Ok(Bytes::from(ciphertext_with_tag))
    }
    ...
    fn encrypt_bytes_stream<R: Read, W: Write, P: AsRef<[u8]> + Send>(
        ...
        while source.take(chunk_size).read_to_end(&mut buf)? == chunk_size as usize {
            let encrypted = encryptor
                ...
                .map_err(EncryptorError::AeadEncryption)?;
    ...
    fn decrypt_bytes_stream<R: Read, W: Write, P: AsRef<[u8]> + Send>(
        ...
        while source.take(chunk_size + 16).read_to_end(&mut buf)? == chunk_size as usize + 16 {
            ...
            let encrypted = encryptor
                ...
                .map_err(EncryptorError::AeadDecryption)?;
            Write::write_all(destination, &encrypted)?;
            written += encrypted.len() as u64;
            buf.clear();
        }
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        let secret = b"my_secret";
        let data = read_to_string(r#"C:\Users\ayfmp\OneDrive\Projects\tenacity-crates\Cargo.lock"#)
            .unwrap();
        let v2_encryptor = V2Encryptor::default();
    ...
    fn test_encrypt_decrypt_base() {
        let data = read_to_string(r#"C:\Users\ayfmp\OneDrive\Projects\tenacity-crates\Cargo.lock"#)
            .unwrap();
        let cipher = V2Encryptor::new()
            .unwrap()
            .base_encrypt_bytes(data.as_bytes())
            .unwrap();
        let decrypted = V2Encryptor::new()
            .unwrap()
            .base_decrypt_bytes(&cipher)
            .unwrap();
        assert_eq!(data.as_bytes(), decrypted.as_ref());
        assert_ne!(decrypted, cipher)
    }
    ...
    fn test_in_place() {
        ...
        let encryptor_test = V2Encryptor {
            salt: [0u8; SALT_SIZE],
        ...
        ciphertext
            .write_all(&(chunk_size as u64).to_be_bytes())
            .unwrap();
        let ciphertext_test = VersionTrait::encrypt_bytes(
            &encryptor_test,
            &[0u8; KEY_SIZE],
            read_to_string("Cargo.toml").unwrap().as_bytes(),
        )
        .unwrap();
        ...
        while (&mut ciphertext)
            .take(chunk_size + 16)
            .read_to_end(&mut buf)
            .unwrap()
            == (chunk_size + 16) as usize
        {
